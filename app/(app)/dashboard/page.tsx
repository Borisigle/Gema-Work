...
export default async function DashboardPage() {
  ...
  const groups = getGroupsByTeacher(session.teacherId);
      ...
      <div className={styles.header}>
        <div className={styles.greeting}>
          ...
          <h1 className={styles.title}>
            Hola, <span className="text-gradient">Profe {session.teacherName}</span>
          </h1>
          <p className={styles.subtitle}>
            {groups.length} {groups.length === 1 ? 'grupo' : 'grupos'} activos · Elegí un grupo para empezar
          </p>
        </div>
      </div>
      ...
      <div className={`${styles.groupsGrid} stagger`}>
        {groups.map((group) => (
          <div key={group.id} className={`${styles.groupCard} card animate-fade-in`}>
            {/* Card Header */}
            <div className={styles.cardHeader} style={{ '--color': color } as React.CSSProperties}>
